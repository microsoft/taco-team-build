...
function parseConfig(newCfg, currentCfg) {
    ...
    // Path to cordova cache
    if (newCfg.cordovaCache) {
        setCachePath(joinAndCreatePath(path.resolve(newCfg.cordovaCache)));
    }
    
    // Version of cordova based node module to use
    if (newCfg.cordovaVersion) currentCfg.cordovaVersion = newCfg.cordovaVersion;
    ...
    if (newCfg.cordovaPackageName) currentCfg.cordovaPackageName = newCfg.cordovaPackageName;
...
function setCachePath(newPath) {
    cordovaCache = joinAndCreatePath(newPath.split(path.sep));
