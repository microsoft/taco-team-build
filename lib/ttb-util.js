...
(function init() {
    ...
    moduleCache = cc;
    ensurePathExists(moduleCache);
    initializeEnvironmentVariables(cc);
})();
...
function ensurePathExists(resolvedPath) {
    return joinAndCreatePath(resolvedPath.split(path.sep))
}
...
function setCachePath(newPath) {
    ...
    initializeEnvironmentVariables(moduleCache);
}

function initializeEnvironmentVariables(newCachePath) {
    process.env['CORDOVA_HOME'] = path.join(newCachePath, '_cordova'); // Set platforms to cache in cache location to avoid unexpected results
    process.env['PLUGMAN_HOME'] = path.join(newCachePath, '_plugman'); // Set plugin cache in cache location to avoid unexpected results
}
