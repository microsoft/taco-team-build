...
function setupCordova(obj) {
	...
	if (!fs.existsSync(cordovaModulePath)) {
		...
		return exec("npm install cordova@" + cordovaVersion, { cwd: cordovaModulePath })
...
function getCordova() {
	...
	if (cdv === undefined || loadedCordovaVersion != cordovaVersion) {
		...
		cdv = require(path.join(cordovaCache, cordovaVersion, "node_modules", "cordova"));
		// Install support plugin if not already present
